...
jQuery(document).ready(function(){
	...
	jQuery('#rut_recupera').Rut({
	  	on_error: function(){ 
	  		alert('Rut incorrecto');
	  		jQuery("#rut_recupera").val('');
	  		jQuery("#rut_recupera").focus();
	  	} 
	});
   ...
   jQuery('<div class="topbar"><a class="barmenu">'+
	...
	jQuery("#btn-olvidar-password").click(function(){
		jQuery.post( "_include/pages/recuperar-password.php", { rut: jQuery("#rut_recupera").val(),email: jQuery("#mail_recupera").val(),fecha_nac: jQuery("#fecha_nac_recupera").val(),ingreso: jQuery("#ingreso_recupera").val()})
			.done(function( data ) {
				jQuery("#form-recupera").hide();
				jQuery("#texto-recupera").show();
				jQuery("#texto-recupera").html(data);
		});
	});
